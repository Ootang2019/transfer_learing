...
class MyInvertedDoublePendulumEnv(MujocoEnv, utils.EzPickle):
    """
    ### Description
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def __init__(self, **kwargs):
        ...
        self.w = np.array([10, 0.01, 1, 1e-3, 5e-3])
    ...
    def step(self, action):
        ...
        features = np.array([1, -(x**2), -((y - 2) ** 2), -(v1**2), -(v2**2)])
        r = np.dot(self.w, features)

        terminated = bool(y <= 1)
        self.renderer.render_step()
        return ob, r, terminated, False, {"features": features}
