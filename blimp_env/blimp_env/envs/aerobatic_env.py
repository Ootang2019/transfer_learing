...
class AerobaticEnv(ROSAbstractEnv):
        ...
        config["target"].update(
            {
                ...
                "task_name": "stand",  # stand, backward, upside_down, loop, roll, vertical_upward, or int(0,5), or random
            }
        )
        config.update(
            {
    ...
    def compute_tacking_rew(self, obs, obs_info: dict) -> float:
        ...
        penalty = -1  # if distance is too far, add extra penalty

        if task == int(0):
            rew = (
                -np.abs(proc_dict["pitch_diff"])
                - np.abs(proc_dict["z_diff"])
                - 0.05 * np.abs(obs_info["velocity"][2])
                - scaled_dist
            )

        elif task == int(1):
            rew = -np.abs(proc_dict["planar_yaw_diff"]) - scaled_dist
        elif task == int(2):
            rew = (
                -np.abs(proc_dict["pitch_diff"])
...
if __name__ == "__main__":
    ...
    env_kwargs = {
        ...
        "target": {
            "task_name": "backward",  # stand, backward, upside_down, loop, roll, or int(0,4), or random
            "DBG_ROS": False,
        },
    }
